...
template<typename Ty, typename Tp>
Array<Ty> approx3(const Array<Ty> &in, const Array<Tp> &pos0, const Array<Tp> &pos1, const Array<Tp> &pos2
                  const af_interp_type method, const float offGrid)
{
    in.eval();
    pos0.eval();
    ...
    case AF_INTERP_NEAREST:
    case AF_INTERP_LOWER:
        getQueue().enqueue(kernel::approx3<Ty, Tp, 1>,
                           out, in, pos0, pos1, pos2, offGrid, method);
        break;
    case AF_INTERP_LINEAR:
    case AF_INTERP_BILINEAR:
    case AF_INTERP_LINEAR_COSINE:
    case AF_INTERP_BILINEAR_COSINE:
        getQueue().enqueue(kernel::approx3<Ty, Tp, 2>,
                           out, in, pos0, pos1, pos2, offGrid, method);
        break;
    case AF_INTERP_CUBIC:
    case AF_INTERP_BICUBIC:
    case AF_INTERP_CUBIC_SPLINE:
    case AF_INTERP_BICUBIC_SPLINE:
        getQueue().enqueue(kernel::approx3<Ty, Tp, 3>,
                           out, in, pos0, pos1, pos2, offGrid, method);
        break;
    default:
        break;
...
#define INSTANTIATE(Ty, Tp)                                                                    \
    ...
    template Array<Ty> approx2<Ty, Tp>(const Array<Ty> &in, const Array<Tp> &pos0,             \
                                       ...
                                       const float offGrid);                                   \
    template Array<Ty> approx2<Ty, Tp>(const Array<Ty> &in, const Array<Tp> &pos0,             \
                                       const Array<Tp> &pos1, const Array<Tp> &pos2,           \
